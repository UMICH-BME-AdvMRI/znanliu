...
sliceGap = 20 * 10^-3; % m
sliceThick = 5 * 10^-3; % m
sliceDistance = sliceGap + sliceThick;
sliceCenter = [-2*sliceDistance, -sliceDistance, sliceDistance, 2*sliceDistance];
sliceFrequence = sliceCenter * gammaBar * gradZ_amp;
rf_carrier = exp(-1i*2*pi*sliceFrequence'*t);
...
for i = 1:length(sliceCenter)
    rf_func = rf_func + rf_func .* rf_carrier(i, :);
end
