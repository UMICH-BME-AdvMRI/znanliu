...
rf_amp = rf_FA / (gammaBar * 1 * pi * rf_area)*10^2;
rf_func = rf_amp * rf_sinc;


gradZ_amp = 18.8*10^-3; % T/m
gradZ = gradZ_amp * ones(1, length(t));
changdu = length(gradZ(1:2:end));
gradZ = [gradZ -1*gradZ(1:2:end)];

rf_func = [rf_func zeros(1, changdu)];
...
figure(2)
hold on; grid on;
plot(tt, MM(1, :), 'Linewidth', 2)
plot(tt, MM(2, :), 'Linewidth', 2)
plot(tt, MM(3, :), 'Linewidth', 2)
plot(tt, abs(Msig), 'Linewidth', 2)
legend('Mx', 'My', 'Mz', 'Mxy', 'Fontsize', 16, 'Location', 'southwest')
xlabel('Time (s)')
