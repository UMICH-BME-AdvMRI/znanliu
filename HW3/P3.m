...
for i = 1:N
    % i-th data
    temp_data = data(:, :, i);
    coil_image(:, :, i) = conj(ifftshift(ifft2(fftshift(temp_data)))) .* coil(:, :, i);
    newKSpace(:, :, i) = fftshift(fft2(ifftshift(coil_image(:, :, i))));
    image = image + coil_image(:, :, i);
end
...
for i = 1:N
    ...
    GRAPPAimage = GRAPPAimage + ifftshift(ifft2(fftshift(GRAPPA_matrix(:, :, i))));
end

figure(1);
imagesc(abs(GRAPPAimage));
title("GRAPPA Recon")
colorbar;
...
figure(2);
imagesc(abs(image) - abs(GRAPPAimage));
title("diff GRAPPA")
colorbar;
